...
import { CodeForgeMobileNav } from "@/components/codeforge/mobile-nav";
...
export default async function CodeForgeLayout({ children }: { children: React.ReactNode }) {
  ...
  const codeforgeDisabled = !settings.codeforgeEnabled && !admin.isAdmin;
    ...
    <div className="flex min-h-[calc(100vh-4rem)]">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="flex items-center justify-between border-b border-border px-5 py-3">
          <CodeForgeMobileNav isAdmin={admin.isAdmin} />
        ...
        <div className="flex-1 p-6">
          {maintenanceBlocked ? (
            ...
            <div className="flex flex-col items-center justify-center gap-3 rounded-lg border border-dashed border-border py-24 text-center">
              <TriangleAlert className="h-8 w-8 text-brass" />
              <h2 className="font-display text-lg font-semibold">CodeForge is temporarily disabled</h2>
              <p className="max-w-sm text-sm text-text-muted">
                An admin has switched CodeForge off for now. Please check back shortly.
              </p>
            </div>
          ) : (
            children
